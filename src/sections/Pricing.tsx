...
function PriceCard({
  ...
  const accent = plan.highlight ? 'text-purple-300' : plan.isDemo ? 'text-emerald-300' : 'text-indigo-300';
  const planId = plan.isDemo ? 'FREE' : plan.highlight ? 'PRO_02' : 'PLAN_01';
      ...
      <Tilt
        className={clsx(
          ...
          plan.highlight
            ...
            : plan.isDemo
              ? 'glass border-emerald-400/25 rounded-2xl'
      ...
      >
        ...
        {plan.isDemo && (
          <div className="absolute -top-4 left-1/2 -translate-x-1/2">
            <span className="badge-popular" style={{ background: 'linear-gradient(135deg, #10b981, #6366f1)' }}>
              Бесплатно
            </span>
          </div>
        )}
        <div className="mb-6">
          <div className="telemetry mb-2">{planId}</div>
        ...
        {!plan.isDemo && (
          <div className="mt-6 text-xs text-gray-500 flex items-center gap-1">
            <Gift className="w-3.5 h-3.5" /> Есть промокод?
          </div>
        )}
        {plan.isDemo ? (
          <button
            onClick={onDemoOpen}
            className="mt-6 btn-primary px-4 py-2.5 rounded-lg text-center text-sm font-medium inline-flex items-center justify-center gap-2"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal>
          <div className="text-center mb-16 max-w-2xl mx-auto">
            ...
            <p className="mt-4 text-gray-400">Попробуй бесплатно прямо в браузере, закажи разовый отчёт или подпишись на регулярные анализы с роадмапом.</p>
          </div>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto items-stretch">
          {plans.map((p, i) => (
            <PriceCard key={p.name} plan={p} delay={i * 0.1} onDemoOpen={() => setDemoOpen(true)} />
          ))}
        </div>
        <Reveal>
